...
build_ivc_vector <- function(
  ...
  # Feedstock and OPEX are ALSO split separately from one another,
  # purely for diagnostic/reporting purposes (separating feedstock-
  # from OPEX-driven NONBIO activation). This does NOT change
  # a_dom/a_imp above, which remain the combined feed+OPEX vectors
  # used everywhere the model actually solves for output - these two
  # extra splits are read-only additions carried forward for later
  # decomposition.
  feed_split <-
    split_dom_imp(
      a_feed
...
# understated monetary input.  In addition, the 2040 configurations
# assigned the whole cached FPBO cost share to each of two model BIO
# sectors, duplicating that intermediate rather than splitting it.
#
# This correction is deliberately made before endpoint construction.
# It preserves, in absolute EUR/t SAF, CAPEX, OPEX, and every non-FPBO
# feedstock component.  Only the residual FPBO intermediate expenditure
# is replaced.  The corrected production cost and cost shares are then
# closed arithmetically, so the resulting technical coefficients carry
# the corrected recurrent input through the economic model.

correct_ivc11a_saf_fpbo_cost <- function(
    fuel_cfg,
    corrected_feed_cost_eur_per_t
) {

  ivc_id <- "IVC11a_SAF"
  bio_intermediates <- c("adv_biodiesel", "adv_biogasoline")

  old_prod_cost <- fuel_cfg$prod_cost[[ivc_id]]
  old_alpha <- fuel_cfg$alpha[[ivc_id]]
  old_dist <- fuel_cfg$dist_feed[[ivc_id]]

  if (
    is.null(old_prod_cost) ||
    is.null(old_alpha) ||
    is.null(old_dist) ||
    !all(c("feed", "capex", "opex") %in% names(old_alpha))
  ) {
    stop("Incomplete IVC11a_SAF scenario cost configuration.")
  }

  intermediate_names <- intersect(bio_intermediates, names(old_dist))
  if (!length(intermediate_names)) {
    stop("IVC11a_SAF has no model BIO sector for its FPBO intermediate.")
  }

  old_feed_cost <- old_prod_cost * old_alpha[["feed"]]
  old_components <- old_feed_cost * old_dist
  non_intermediate_names <- setdiff(names(old_components), intermediate_names)
  non_intermediate_cost <- sum(old_components[non_intermediate_names])
  corrected_fpbo_cost <- corrected_feed_cost_eur_per_t - non_intermediate_cost

  if (
    !is.finite(corrected_feed_cost_eur_per_t) ||
    corrected_feed_cost_eur_per_t <= 0 ||
    !is.finite(corrected_fpbo_cost) ||
    corrected_fpbo_cost <= 0
  ) {
    stop("Invalid corrected IVC11a_SAF feedstock or FPBO cost.")
  }

  # Preserve the pre-existing mapping between the workbook's single FPBO
  # row and model BIO sectors, but make those sector shares sum to the one
  # corrected FPBO amount.  This also removes the 2040 double allocation.
  intermediate_split <- old_components[intermediate_names]
  if (any(intermediate_split < 0) || sum(intermediate_split) <= 0) {
    stop("Invalid IVC11a_SAF FPBO sector allocation.")
  }
  intermediate_split <- intermediate_split / sum(intermediate_split)

  corrected_components <- old_components
  corrected_components[intermediate_names] <-
    corrected_fpbo_cost * intermediate_split

  old_capex_cost <- old_prod_cost * old_alpha[["capex"]]
  old_opex_cost <- old_prod_cost * old_alpha[["opex"]]
  corrected_prod_cost <-
    corrected_feed_cost_eur_per_t + old_capex_cost + old_opex_cost

  fuel_cfg$prod_cost[[ivc_id]] <- corrected_prod_cost
  fuel_cfg$alpha[[ivc_id]] <- c(
    feed = corrected_feed_cost_eur_per_t,
    capex = old_capex_cost,
    opex = old_opex_cost
  ) / corrected_prod_cost
  fuel_cfg$dist_feed[[ivc_id]] <-
    corrected_components / corrected_feed_cost_eur_per_t
...
# Correct totals recomputed from workbook rows 69:78 as
# sum(mix share * EUR/t feedstock / t fuel per t feedstock).
IVC11A_SAF_CORRECTED_FEED_COST_EUR_PER_T <- c(
  F = 670.7543057818430,
  I = 682.0541117568853,
  K = 703.6097408241184,
  M = 709.2596438116395
...
S1_2030$adv_bio_kerosene <- correct_ivc11a_saf_fpbo_cost(
  S1_2030$adv_bio_kerosene,
  IVC11A_SAF_CORRECTED_FEED_COST_EUR_PER_T[["F"]]
)
S1_2035$adv_bio_kerosene <- correct_ivc11a_saf_fpbo_cost(
  S1_2035$adv_bio_kerosene,
  IVC11A_SAF_CORRECTED_FEED_COST_EUR_PER_T[["F"]]
)
S2_2035$adv_bio_kerosene <- correct_ivc11a_saf_fpbo_cost(
  S2_2035$adv_bio_kerosene,
  IVC11A_SAF_CORRECTED_FEED_COST_EUR_PER_T[["I"]]
)
S3_2035$adv_bio_kerosene <- correct_ivc11a_saf_fpbo_cost(
  S3_2035$adv_bio_kerosene,
  IVC11A_SAF_CORRECTED_FEED_COST_EUR_PER_T[["I"]]
)
S1_2040$adv_bio_kerosene <- correct_ivc11a_saf_fpbo_cost(
  S1_2040$adv_bio_kerosene,
  IVC11A_SAF_CORRECTED_FEED_COST_EUR_PER_T[["K"]]
)
S2_2040$adv_bio_kerosene <- correct_ivc11a_saf_fpbo_cost(
  S2_2040$adv_bio_kerosene,
  IVC11A_SAF_CORRECTED_FEED_COST_EUR_PER_T[["M"]]
)
S3_2040$adv_bio_kerosene <- correct_ivc11a_saf_fpbo_cost(
  S3_2040$adv_bio_kerosene,
  IVC11A_SAF_CORRECTED_FEED_COST_EUR_PER_T[["M"]]
)
...
# Feedstock- and OPEX-only diagnostic matrices, same role as
# A_capex_dom_target above: this function builds a scenario endpoint
# (2030/2035/2040) from the IVC technology library, so these start at
# zero and are filled in for BIO columns below. BASE_2023's own
# A_feed_dom/A_opex_dom (constructed separately, see BASE_2023 below)
# provide the real 2023 starting point for interpolation.
A_feed_dom_target <-
  matrix(
    0,
    nrow = nIndustries,
...
if (is.null(sourcing_group)) {
 ...
 res_fuel <-
  ...
  res_fuel$gate_fee_coeff
    ...
    # -------------------------------------------------------------
    # Feedstock- and OPEX-only diagnostic coefficients. Same fill
    # pattern as CAPEX above - read-only additions, never written
    # into A_dom_target / A_imp_target (which remain the combined
    # feed+OPEX matrices used for the actual solve).
    # -------------------------------------------------------------

    A_feed_dom_target[, target_col] <- 0
    A_feed_imp_target[, target_col] <- 0
    A_opex_dom_target[, target_col] <- 0
    A_opex_imp_target[, target_col] <- 0

    # Domestic feedstock coefficients
    ...
    # Imported feedstock coefficients
    for (imp_sec in names(res_fuel$a_feed_imp)) {
      ...
      if (clean_sec %in% names(INPUT_SECTORS)) {
        ...
        A_feed_imp_target[row_idx, target_col] <-
          res_fuel$a_feed_imp[[imp_sec]]
      }
    }

    # Domestic OPEX coefficients
    ...
    # Imported OPEX coefficients
    for (imp_sec in names(res_fuel$a_opex_imp)) {
  ...
  # -----------------------------------------------------------------
  # SAFETY CHECK: the combined recurrent matrix used in the solve
  # must exactly equal the sum of its feedstock and OPEX components,
  # for every BIO column built in this endpoint.
  # -----------------------------------------------------------------
...
# -------------------------------------------------------------------
# Feedstock- and OPEX-only diagnostic coefficients for 2023 (Section
# 4.2 analysis: separating feedstock- from OPEX-driven NONBIO
# activation, extending the same decomposition back to the observed
# 2023 economy). Derived the same way as A_capex_dom_BASE_2023 above:
# from the '2023' sheet's own TOTALS domestic / TOTAL imported blocks,
# coefficient = absolute EUR value / total 2023 market value of that
# fuel. OPEX import shares applied via the same OPEX_IMPORT_SHARES
# used for scenario years; feedstock domestic/imported is read
# directly from the sheet's own domestic/imported split.
#
# STATUS: successfully derived for adv_biodiesel, adv_biogasoline and
# conv_biodiesel (the three fuels with real 2023 production and a
# clean 'TOTALS domestic' / 'TOTAL imported' block in the source
# sheet). adv_biogas (column 33) and conv_biogasoline/conv_ethanol
# (column 14) still need their own equivalent extraction - the 2023
# sheet's summary block for these two uses a slightly different label
# format ('TOTALS ' with inconsistent spacing) that this extraction
# does not yet handle. They are left at zero for now, exactly like
# A_capex_dom_BASE_2023 was left at zero for fuels with no 2023 data.

A_feed_dom_BASE_2023 <-
  matrix(
    0,
    nrow = nIndustries,
    ncol = nIndustries,
...
# --- adv_biodiesel (column 13) ---
A_feed_dom_BASE_2023[1, 13]  <- 0.00121784  # agriculture
A_feed_dom_BASE_2023[8, 13]  <- 0.00038355  # paper
A_feed_dom_BASE_2023[5, 13]  <- 0.00002739  # food_bev
A_feed_dom_BASE_2023[35, 13] <- 0.00005303  # sewerage
A_feed_dom_BASE_2023[2, 13]  <- 0.00045189  # forestry
A_feed_imp_BASE_2023[5, 13]  <- 0.56565853  # food_bev_imp
A_opex_dom_BASE_2023[32, 13] <- 0.04951068  # electricity
A_opex_dom_BASE_2023[11, 13] <- 0.02516355  # chemicals
A_opex_dom_BASE_2023[54, 13] <- 0.02875834  # legal_acc
A_opex_dom_BASE_2023[31, 13] <- 0.02218107  # repair_inst
A_opex_dom_BASE_2023[40, 13] <- 0.00739369  # land_transp
A_opex_imp_BASE_2023[11, 13] <- 0.01078438  # chemicals_imp

# --- adv_biogasoline (column 15) ---
A_feed_dom_BASE_2023[1, 15]  <- 0.24225323  # agriculture
A_feed_dom_BASE_2023[8, 15]  <- 0.06182032  # paper
A_feed_dom_BASE_2023[5, 15]  <- 0.00024760  # food_bev
A_feed_dom_BASE_2023[35, 15] <- 0.00047933  # sewerage
A_feed_dom_BASE_2023[2, 15]  <- 0.00408485  # forestry
A_opex_dom_BASE_2023[32, 15] <- 0.07148462  # electricity
A_opex_dom_BASE_2023[11, 15] <- 0.12186935  # chemicals
A_opex_dom_BASE_2023[54, 15] <- 0.05684220  # legal_acc
A_opex_dom_BASE_2023[31, 15] <- 0.04404476  # repair_inst
A_opex_dom_BASE_2023[40, 15] <- 0.02017739  # land_transp
A_opex_imp_BASE_2023[11, 15] <- 0.05222972  # chemicals_imp

# --- conv_biodiesel (column 12) ---
A_feed_dom_BASE_2023[5, 12]  <- 0.50977573  # food_bev
A_feed_imp_BASE_2023[5, 12]  <- 0.07139994  # food_bev_imp
A_opex_dom_BASE_2023[11, 12] <- 0.04871839  # chemicals
A_opex_dom_BASE_2023[54, 12] <- 0.03627699  # legal_acc
A_opex_dom_BASE_2023[40, 12] <- 0.01380397  # land_transp
A_opex_imp_BASE_2023[11, 12] <- 0.00541315  # chemicals_imp
...
build_scenario_driver <- function(
  ...
  # Feedstock- and OPEX-only diagnostic paths, kept separate
  # throughout in the same way as the CAPEX path above.
...
exports_end   <- end_endpoint$exports
  ...
  for (i in seq_along(years)) {
    ...
    # -------------------------------------------------------------
    # Feedstock- and OPEX-only diagnostic matrices, interpolated
    # exactly like the CAPEX matrices above.
    # -------------------------------------------------------------

    A_feed_dom_current <- start_endpoint$A_feed_dom
    A_feed_imp_current <- start_endpoint$A_feed_imp
    A_opex_dom_current <- start_endpoint$A_opex_dom
    A_opex_imp_current <- start_endpoint$A_opex_imp

    A_feed_dom_current[, BIO] <-
      start_endpoint$A_feed_dom[, BIO, drop = FALSE] +
      lambda * (
        end_endpoint$A_feed_dom[, BIO, drop = FALSE] -
          start_endpoint$A_feed_dom[, BIO, drop = FALSE]
      )

    A_feed_imp_current[, BIO] <-
      start_endpoint$A_feed_imp[, BIO, drop = FALSE] +
      lambda * (
        end_endpoint$A_feed_imp[, BIO, drop = FALSE] -
          start_endpoint$A_feed_imp[, BIO, drop = FALSE]
      )

    A_opex_dom_current[, BIO] <-
      start_endpoint$A_opex_dom[, BIO, drop = FALSE] +
      lambda * (
        end_endpoint$A_opex_dom[, BIO, drop = FALSE] -
          start_endpoint$A_opex_dom[, BIO, drop = FALSE]
      )

    A_opex_imp_current[, BIO] <-
      start_endpoint$A_opex_imp[, BIO, drop = FALSE] +
      lambda * (
        end_endpoint$A_opex_imp[, BIO, drop = FALSE] -
          start_endpoint$A_opex_imp[, BIO, drop = FALSE]
      )
...
combine_scenario_drivers <- function(...) {
  ...
  for (k in 2:length(drivers)) {
    ...
    # ------------------------------------------------------------
    # A_feed_dom / A_feed_imp / A_opex_dom / A_opex_imp
    # Feedstock- and OPEX-only diagnostic paths. Must be carried
    # forward across combined segments for exactly the same reason
    # as A_capex_dom/A_capex_imp above.
    # ------------------------------------------------------------
    ...
    A_feed_dom_new[1:old_n, , ] <- out$A_feed_dom
    ...
    A_feed_imp_new[1:old_n, , ] <- out$A_feed_imp
    ...
    A_opex_dom_new[1:old_n, , ] <- out$A_opex_dom
    ...
    A_opex_imp_new[1:old_n, , ] <- out$A_opex_imp
